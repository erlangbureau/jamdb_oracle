defmodule Jamdb.OracleTest do
  ...
  test "insert with typed returning fields" do
    query = insert(nil, "schema", [:x], [[:x]], {:raise, [], []}, [{:id, :id}])
    assert query == ~s{INSERT INTO schema (x) VALUES (:1) RETURN id INTO :id}
  end

  test "infers output bind types for Ecto returning fields" do
    schema_meta = %{schema: Schema}

    assert Ecto.Adapters.Jamdb.Oracle.returning_options([], schema_meta, [:id, :x, :w]) ==
      [out: [:integer, :integer, :decimal]]
    assert Ecto.Adapters.Jamdb.Oracle.returning_options([out: [:number]], schema_meta, [:id]) ==
      [out: [:number]]
    assert Ecto.Adapters.Jamdb.Oracle.returning_options([], %{schema: nil},
      [{:id, :id}, {:name, :string}, {:uuid, :binary_id}]) ==
      [out: [:integer, :string, :binary]]
  end
  ...
  test "create table with reference" do
    ...
    assert execute_ddl(create) == [
      """
      CREATE TABLE posts (id integer generated by default as identity, category_0 int, CONSTRAINT posts_category_0_fkey FOREIGN KEY (category_0) REFERENCES categories(id),
  ...
  test "create table with options" do
    ...
    assert execute_ddl(create) == [
      """
      CREATE TABLE posts (id integer generated by default as identity, created_at timestamp(0), CONSTRAINT posts_pkey PRIMARY KEY (id))
      TABLESPACE FOO
      """
      |> remove_newlines
    ]
  ...
  test "serial columns are identities only when used as primary keys" do
    create =
      {:create, table(:posts),
       [
         {:add, :id, :bigserial, [primary_key: true]},
         {:add, :sequence_number, :serial, []}
       ]}

    assert execute_ddl(create) == [
      "CREATE TABLE posts (id integer generated by default as identity, " <>
      "sequence_number int, CONSTRAINT posts_pkey PRIMARY KEY (id))"
    ]
  end
